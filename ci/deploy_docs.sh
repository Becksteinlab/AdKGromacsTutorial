...
###############################################################################
# This file has been copied from the MDAnalysisTutorial and should be modified
# for the AdK Gromacs Tuturial!!

# In particular, must set GH_REPOSITORY and ADK_DOCDIR in .travis.yml
###############################################################################
...
test -n "${ADK_DOCDIR}" || die "ADK_DOCDIR must be set in .travis.yml" 100

cd ${DOCDIR} || die "Failed to 'cd ${MDA_DOCDIR}'. Run from the top level of the repository"
